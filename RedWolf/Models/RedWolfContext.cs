...
namespace RedWolf.Models
{
    public class RedWolfContext : IdentityDbContext<RedWolfUser>
    {
        ...
        public DbSet<Grawl> Grawls { get; set; }
        public DbSet<GrawlTask> GrawlTasks { get; set; }
        public DbSet<GrawlTaskAuthor> GrawlTaskAuthors { get; set; }
        ...
        public DbSet<GrawlCommand> GrawlCommands { get; set; }
        public DbSet<CommandOutput> CommandOutputs { get; set; }
        public DbSet<GrawlTasking> GrawlTaskings { get; set; }
        ...
        public RedWolfContext(DbContextOptions<RedWolfContext> options) : base(options)
        {
        ...
        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
            => optionsBuilder
                .UseSqlite("Data Source=" + Common.RedWolfDatabaseFile);

        protected override void OnModelCreating(ModelBuilder builder)
        {
            builder.Entity<GrawlTaskOption>().ToTable("GrawlTaskOption");
            ...
            builder.Entity<Grawl>()
                .HasOne(G => G.ImplantTemplate)
                .WithMany(IT => IT.Grawls)
                .HasForeignKey(G => G.ImplantTemplateId);

            builder.Entity<GrawlTask>()
                .HasOne(GT => GT.Author)
                .WithMany(GTA => GTA.GrawlTasks)
                .HasForeignKey(GT => GT.AuthorId);

            builder.Entity<GrawlCommand>()
                .HasOne(GC => GC.GrawlTasking)
                .WithOne(GT => GT.GrawlCommand)
                .HasForeignKey<GrawlCommand>(GC => GC.GrawlTaskingId)
                .IsRequired(false);

            builder.Entity<GrawlCommand>()
                .HasOne(GC => GC.CommandOutput)
                .WithOne(CO => CO.GrawlCommand)
                .HasForeignKey<GrawlCommand>(GC => GC.CommandOutputId);
            ...
            builder.Entity<GrawlTaskReferenceAssembly>()
                .HasKey(t => new { t.GrawlTaskId, t.ReferenceAssemblyId });
            builder.Entity<GrawlTaskReferenceAssembly>()
                .HasOne(gtra => gtra.GrawlTask)
                .WithMany("GrawlTaskReferenceAssemblies");
            builder.Entity<GrawlTaskReferenceAssembly>()
                .HasOne(gtra => gtra.ReferenceAssembly)
                .WithMany("GrawlTaskReferenceAssemblies");

            builder.Entity<GrawlTaskEmbeddedResource>()
                .HasKey(t => new { t.GrawlTaskId, t.EmbeddedResourceId });
            builder.Entity<GrawlTaskEmbeddedResource>()
                .HasOne(gter => gter.GrawlTask)
                .WithMany("GrawlTaskEmbeddedResources");
            builder.Entity<GrawlTaskEmbeddedResource>()
                .HasOne(gter => gter.EmbeddedResource)
                .WithMany("GrawlTaskEmbeddedResources");

            builder.Entity<GrawlTaskReferenceSourceLibrary>()
                .HasKey(t => new { t.GrawlTaskId, t.ReferenceSourceLibraryId });
            builder.Entity<GrawlTaskReferenceSourceLibrary>()
                .HasOne(gtrsl => gtrsl.GrawlTask)
                .WithMany("GrawlTaskReferenceSourceLibraries");
            builder.Entity<GrawlTaskReferenceSourceLibrary>()
            ...
            builder.Entity<Grawl>().Property(G => G.Children).HasConversion
            (
                v => JsonConvert.SerializeObject(v),
                v => v == null ? new List<string>() : JsonConvert.DeserializeObject<List<string>>(v)
            ).Metadata.SetValueComparer(stringListComparer);

            builder.Entity<GrawlTask>().Property(GT => GT.Aliases).HasConversion
            (
                v => JsonConvert.SerializeObject(v),
                v => v == null ? new List<string>() : JsonConvert.DeserializeObject<List<string>>(v)
            ).Metadata.SetValueComparer(stringListComparer);

            builder.Entity<GrawlTask>().Property(GT => GT.CompatibleDotNetVersions).HasConversion
            (
                v => JsonConvert.SerializeObject(v),
                v => v == null ? new List<Common.DotNetVersion>() : JsonConvert.DeserializeObject<List<Common.DotNetVersion>>(v)
            ).Metadata.SetValueComparer(dotnetversionIListComparer);

            builder.Entity<GrawlTaskOption>().Property(GTO => GTO.SuggestedValues).HasConversion
            (
                v => JsonConvert.SerializeObject(v),
                v => v == null ? new List<string>() : JsonConvert.DeserializeObject<List<string>>(v)
            ).Metadata.SetValueComparer(stringListComparer);

            builder.Entity<GrawlTasking>().Property(GT => GT.Parameters).HasConversion
            (
                v => JsonConvert.SerializeObject(v),
                v => v == null ? new List<string>() : JsonConvert.DeserializeObject<List<string>>(v)
            ).Metadata.SetValueComparer(stringListComparer);

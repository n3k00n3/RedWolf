    ...
    public class DownloadEvent : Event
    {
        ...
        public bool WriteToDisk()
        {
            try
            {
                ...
                {
                    File.WriteAllBytes(
                        Path.Combine(Common.RedWolfDownloadDirectory, Utilities.GetSanitizedFilename(this.FileName)),
                        contents
                    );
                }
                else
                {
                    using (var stream = new FileStream(Path.Combine(Common.RedWolfDownloadDirectory, Utilities.GetSanitizedFilename(this.FileName)), FileMode.Append))
                    {

...
namespace RedWolf.Models.Listeners
{
    public class BridgeListener : Listener
    {
        ...
        public override CancellationTokenSource Start()
        {
            ...
            _ = this.InternalListener.Configure(InternalListener.ToProfile(this.Profile), this.ANOTHERID, this.RedWolfUrl, this.RedWolfToken);
        ...
        private void NetworkWriteString(NetworkStream stream, string data)
        {
            if (!string.IsNullOrEmpty(data))
            {
                byte[] dataBytes = Common.RedWolfEncoding.GetBytes(data);
        ...
        private string NetworkReadString(NetworkStream stream, CancellationToken token)
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                return Common.RedWolfEncoding.GetString(ms.ToArray());
            }
        }
    }
}

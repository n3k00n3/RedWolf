...
namespace RedWolf.Core
{
    public static class Compiler
    {
        ...
        {
            public RedWolf.Models.Grawls.ImplantLanguage Language { get; set; } = Models.Grawls.ImplantLanguage.CSharp;
        ...
        private static byte[] ConfuseAssembly(byte[] ILBytes)
        {
            ...
            File.WriteAllBytes(Common.RedWolfTempDirectory + "confused", ILBytes);
            string ProjectFile = String.Format(
                ConfuserExOptions,
                Common.RedWolfTempDirectory,
                Common.RedWolfTempDirectory,
                "confused"
            );
            doc.Load(new StringReader(ProjectFile));
            project.Load(doc);
            project.ProbePaths.Add(Common.RedWolfAssemblyReferenceNet35Directory);
            project.ProbePaths.Add(Common.RedWolfAssemblyReferenceNet40Directory);

            ConfuserParameters parameters = new ConfuserParameters();
            parameters.Project = project;
            parameters.Logger = default;
            ConfuserEngine.Run(parameters).Wait();
            return File.ReadAllBytes(Common.RedWolfTempDirectory + "confused");
        }
